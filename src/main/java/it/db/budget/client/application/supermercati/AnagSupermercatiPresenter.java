...
public class AnagSupermercatiPresenter extends Presenter<AnagSupermercatiPresenter.MyView, AnagSupermercatiPresenter.MyProxy> implements AnagSupermercatiUiHandlers {

	PlaceManager placeManager;
	
	interface MyView extends View, HasUiHandlers<AnagSupermercatiPresenter> {
		//void setDataSourceName(String aReportName);
	}
	
	@ProxyStandard
	@NameToken(NameTokens.ANAG_SUPERMERCATI)
	interface MyProxy extends ProxyPlace<AnagSupermercatiPresenter> {
	}
	
	@Inject
	AnagSupermercatiPresenter(EventBus eventBus, MyView view, MyProxy proxy, PlaceManager aPlaceManager) {
